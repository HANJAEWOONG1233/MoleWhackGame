...
3.  **Target Architectures 설정**  
    - **ARMv7 (32비트)**: 대부분 기기 호환  
    - **ARM64 (64비트)**: 최신 기기 권장  
    - 안전을 위해 **둘 다** 체크  
4.  **Scripting Backend 변경**  
    1. Unity 상단 메뉴에서 **Edit > Project Settings** 클릭  
    2. 왼쪽에서 **Player** 선택 후 Inspector 창에서 **Android** 탭 클릭  
    3. **Other Settings** 섹션에서 **Scripting Backend**를 **IL2CPP**로 변경  
    4. 다시 **Target Architectures**에서 **ARMv7**과 **ARM64**를 모두 체크  
5.  `Build` 버튼 → `.apk` 파일 생성 → 기기에 설치.  
